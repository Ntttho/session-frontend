...
function showpProduct(listProduct) {
...
showpProduct(products);

function search(listProduct) {
  let arrProduct = [];
  ...
  btn.addEventListener("click", () => {
    ...
    if (content !== "") {
      arrProduct = listProduct.filter((element) => {
        return element.name.includes(content);
      });
      showpProduct(arrProduct);
    } else {
      showpProduct(listProduct);
    }
    input.value = "";
  });
}
